...
public class SlayerKit extends Kit {
	...
	public SlayerKit(ItemStack item) {
		...
		addAction(PlayerInteractEvent.class, "Slayer-SwordDash", event -> {
			...
			if (p.getItemInUse() == null)
				return;
			if (p.getItemInUse().getType() != Material.STONE_SWORD)
				return;
	...
	public void rg(final Player p, final int start, final int rad) {
		final ZombieSurvival zs = JavaPlugin.getPlugin(ZombieSurvival.class);
		Bukkit.getScheduler().runTaskLater(zs, new Runnable() {
			@Override
			public void run() {
				if (start < rad) {
					if (p.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.AIR) {
						for (final Block b : circle(p, start)) {
							b.getWorld().playEffect(b.getLocation(), Effect.STEP_SOUND, b.getType());
							final FallingBlock fb = b.getWorld().spawnFallingBlock(b.getLocation(), b.getState().getBlockData());
							fb.setHurtEntities(false);
							fb.setVelocity(new Vector(0, .35, 0));

							//Location loc = b.getLocation().add(0, 1, 0);
							final List<Entity> ents = fb.getNearbyEntities(1, 1, 1);
							if (ents.size() > 0) {
								for (final Entity e : ents) {
									...
									if (e instanceof FallingBlock)
										continue;
									final Vector direction = e.getLocation().toVector().add(fb.getLocation().toVector().multiply(-1));
									if (p.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() == Material.AIR)
										continue;
									direction.setY(0.5);
