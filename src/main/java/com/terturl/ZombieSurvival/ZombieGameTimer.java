...
public class ZombieGameTimer extends MinigameRunnerTimerExecution {
	...
	public void runCleanup() {
		if (((List<UUID>) getActiveGame().getAdditionalInformation().getList("ZombiesInArena")).size() >= 1) {
			for (final UUID uuid : (List<UUID>) getActiveGame().getAdditionalInformation().getList("ZombiesInArena")) {
	...
	public void inGameRunningMethod(Integer gameTime) {
		...
		if (firstLaunch) {
			...
			for (int zombie = 0; zombie < zombies; zombie++) {
				final int xMin = Math.min((int) start.getX(), (int) end.getX());
				final int xMax = Math.max((int) start.getX(), (int) end.getX());
				final int zMin = Math.min((int) start.getZ(), (int) end.getZ());
				final int zMax = Math.max((int) start.getZ(), (int) end.getZ());
				final int x = r.nextInt(xMax - xMin) + xMin;
				final int z = r.nextInt(zMax - zMin) + zMin;
				final int y = start.getWorld().getHighestBlockYAt(x, z);
		...
		if (getActiveGame().getAdditionalInformation().getList("ZombiesInArena").size() == 0) {
			...
			final double twoPerc = zombies * 0.2;
	...
	public void spawnZombie(Location loc) {
		...
		final int i = scheduler.scheduleSyncRepeatingTask(ZombieSurvival.getInstance(), new Runnable() {
			...
			public void run() {
				if (j < 22) {
					j++;
					final Block b = z.getLocation().getBlock();
					final Block tmp = Bukkit.getWorld(b.getWorld().getName()).getHighestBlockAt(b.getLocation());
